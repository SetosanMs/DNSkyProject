...
// if dumped
inline void ErasePEHeaderFromMemory(LPCSTR handle)
{
	//DWORD OldProtect = 0;

	// Get base address of module
	//	DWORD *pBaseAddr = (DWORD*)GetModuleHandle(handle);

	//	SetRange((LPVOID)0x400000, 1, ASM::NOP);
	//printf("ADDR = %.2X \n", GetModuleHandle(NULL));
	/*// Change memory protection
	VirtualProtect(pBaseAddr, 2, // Assume x86 page size
	PAGE_READWRITE, &OldProtect);

	// Erase the header
	ZeroMemory(pBaseAddr, 2);*/
}
