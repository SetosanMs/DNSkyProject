...
wchar_t transChars[TRANS_CHARS_LENGTH] =
{
...
static bool CharacterMapContains(wchar_t c)
{
...
static CharRep GetCharRep(wchar_t c)
{
	for (int i = 0; i < MAP_LENGTH; i++)
	{
		if (charsMap[i].code == c)
...
static CombCharRep GetCombCharRep(wchar_t c1, wchar_t c2)
{
	int i = 0;
...
static bool IsTransparent(wchar_t c)
{
	int i = 0;
...
wchar_t* shape_arabic(wchar_t * str, int len)
{
	CharRep crep;
