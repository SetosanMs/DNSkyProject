...
bool is_arabic_letter(uint32_t cp){ return ( cp >= ARABIC_LETTER_START && cp <=  ARABIC_LETTER_END ); }
bool is_lam_alef(uint32_t cp, uint32_t next){ return cp == UNICODE_LAM &&
									 is_arabic_letter(next) &&
									 arabic_forms_b[next - ARABIC_LETTER_START][1][INITIAL] != 0; }
bool is_alef_prev_lam(uint32_t prev, uint32_t cp)	{ return prev == UNICODE_LAM &&
									 is_arabic_letter(cp) &&
									 arabic_forms_b[cp - ARABIC_LETTER_START][1][INITIAL] != 0; }

bool is_linking_type(uint32_t cp) { return is_arabic_letter(cp) && arabic_forms_b[cp - ARABIC_LETTER_START][0][MEDIAL] != 0; }
